...
// Shared nyaa.si source factory. The English "Nyaa" source searches the
// English-translated category (1_2) with no language filter; "Nyaa (Español)"
// searches the non-English category (1_3) and only keeps releases whose title
// carries a Spanish marker.
export function createNyaaSource ({ category = '1_2', resultFilter = null } = {}) {
  ...
  async function search (query, options, kind) {
    ...
    if (resultFilter) {
      filtered = filtered.filter(r => resultFilter(r.title))
    }
    return filtered
  }
